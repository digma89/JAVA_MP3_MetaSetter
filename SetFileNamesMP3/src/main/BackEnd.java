		...
		File[] listOfFiles = folder.listFiles();
		    for (int i = 0; i < listOfFiles.length; i++) {
		      if (listOfFiles[i].isFile()) {
		        ...
		        setMetaData(listOfFiles[i].getName().trim(), folerSource, folderResult);	
		      }
		    }
	}
	
	//With the file name set the MetaData of the files
	private void setMetaData(String fileName, String folerSource, String folderResult) throws BaseException, Exception, IOException{		
